...
class ExtractFeaturesStage(BaseLoaderStage):
    ...
    def get_column_specs(self, *, owner, state: MutableMapping[str, Any]) -> Sequence[NDArrayColumnSpec]:
        _ = state
    ...
    def get_custom_extractors(self, *, owner, state: MutableMapping[str, Any]) -> Sequence[CustomExtractor]:
        _ = owner
        _ = state
    ...
    def run_loader(self, *, state: MutableMapping[str, Any], owner) -> None:
        ...
        if self.only_if_include_targets and not bool(getattr(owner, "include_targets", False)):
        ...
        for spec in self.get_column_specs(owner=owner, state=state):
            self._extract_column(table=table, spec=spec, store=out_store)

        for extractor in self.get_custom_extractors(owner=owner, state=state):
            extra = extractor(table, owner, state)
            if extra:
